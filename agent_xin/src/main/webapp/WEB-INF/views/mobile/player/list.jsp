...
<nav class="breadcrumb clearfix">玩家列表</nav>
    <div class="page-container">
        <div class="mt-20 clearfix" >
           <div style="width:100%;" class="personP" >
               <div class="dataTime">
                  <label class="search"><input id="searchIN" type="text" name=""  class="input-text" placeholder="请输入UID"><button onclick="initPage()" type="button" class="btn btn-success radius" id="searchBT" name=""><i class="Hui-iconfont">&#xe665;</i> 搜索</button></label>
	    ...
	    $('.pageContent').dropload({
	        ...
	        loadDownFn : function(me){
	        	var searchStr=$("#searchIN").val().trim();
		            ...
		            $.ajax({
		                type: 'GET',
		                url: "<c:url value='/userinfo/mPlayerData'/>",
		                    ...
		                    if(dataList.length > 0){
		                    	  for(var i=0; i<dataList.length; i++){
		                    		  result +='<ul class="ul floatL hover">'
			   	                          ...
			   	                          +'<li>'+dataList[i].nickName+'</li>'
			   	                          +'<li>'+dataList[i].dispalyBindTime+'</li>'
