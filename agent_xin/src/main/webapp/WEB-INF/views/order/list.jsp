				...
				<tbody>
				     <c:forEach var="order" items="${orderList}" varStatus="status"> 
                       <tr class="text-c" >
        		        ...
        		        <td>${order.guid}</td>
        		        <td>${order.orderId}</td>
        		        <td>${order.superGuid}</td>
        		        <td>${order.wares}</td>
        		        <td>${order.totalFee/100}元</td>
        		        <td><fmt:formatDate pattern="yyyy-MM-dd HH:mm:ss" value="${order.payTime}"/></td>
					   </tr>
                    </c:forEach> 	
				</tbody>
			</table>
...
$(function(){
    $("#search").click(function(){
    	$("#stateForm").action="<c:url value='/order/list'/>";
    	$("#stateForm").submit();
    })
})
</script>
