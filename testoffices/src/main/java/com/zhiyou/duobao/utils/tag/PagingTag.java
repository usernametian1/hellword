...
public class PagingTag extends TagSupport {
    ...
    private String show() {
        ...
        if(!paging.isFirst()) {
            buf.append("<a  onclick=\"javascript:onPaging('" + formId + "','" + requetURI + "','" + (paging.getCurPageNum() - 1) + "');\">上一页</a>");
        } 
        
        int[] pageNumArray = paging.getPageNumArray();
        for (int i = 0; i < pageNumArray.length; i++) {
            if (pageNumArray[i] == paging.getCurPageNum()) {
                buf.append("<span class=\"hover\" onclick=\"javascript:void(0);\" >" + pageNumArray[i] + "</span>");
            } else {
                buf.append("<a onclick=\"javascript:onPaging('" + formId + "','" + requetURI + "','" + pageNumArray[i] + "');\">" + pageNumArray[i] + "</a>");
            }
        }
        
        if (!paging.isLast()) {
        	 buf.append("<a onclick=\"javascript:onPaging('" + formId + "','" + requetURI + "','" + (paging.getCurPageNum() + 1) + "');\">下一页</a>");
        }
